...
6. law：法律全名（例如「醫療法」），不是 law_article 就給空字串。article：條號（例如「第106條」），不是 law_article 就給空字串。
   bill_keywords：議案名稱裡的正式用詞，越短越好，可以用空白分隔多個；不是議案就給空字串。修法草案用「被修的法律名稱」（例如「醫療法」），條例草案用條例名稱裡的詞（例如「無人載具」而不是口語的「無人機」）。不要放政黨或提案者，那是 proposer。
   proposer：那個數字所屬版本的提案者，不是發言的委員本人；委員說「行政院的版本」「國民黨版」
   「我們黨團提的」就填對應的提案者（例如「行政院」「國民黨黨團」「民進黨黨團」「台灣民眾黨黨團」
   或提案委員姓名），說不出是誰的版本就給空字串——委員質詢時常拿好幾個版本的數字互相比較，
   不能預設都是他自己提的。
7. 只挑「質詢委員本人」說的話。委員會質詢裡官員的回答（常以「報告委員」「謝謝委員」開頭，或是部長、次長、署長的說明）一律不要挑。
8. 最多 {MAX_CLAIMS} 則。沒有可查證的陳述就回傳空陣列。"""
...
注意：證據裡的「查詢時的議案狀態」是現在的狀態，不一定是發言當時的；主張講的是發言日期當時的情況。
...
def _body(model: str, num_ctx: int, messages: list[dict], schema: dict) -> dict:
    return {
        ...
        "format": schema,
        "stream": False,
        # 關掉思考：qwen3.5 是思考型模型，不關的話會把整段時間拿去推理，最後回空的
        # content（實測一篇逐字稿跑九分鐘、回應是空的；關掉後 8–18 秒）
        "think": False,
