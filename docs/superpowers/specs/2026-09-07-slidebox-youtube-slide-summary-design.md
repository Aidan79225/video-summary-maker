...
- **WebP 沒有被重用**：`_attach_images` 沒有檔案存在檢查，重跑一律重新下載片段、重新抽幀；舊的 WebP 會被同名覆蓋，但從未被讀取來省下這些步驟。快取重用是未實作的項目，不是目前的行為。
- **取消在摘要階段是延遲生效，不是立即生效**：`Summarizer.summarize()` 這個 port 沒有 `CancelCheck` 參數，使用者在生成摘要期間按下取消，要等這一步完成（NDJSON 串流跑完或逾時／出錯）才會真的中止，最長可能是一次約 122 秒執行裡的 100 秒左右。這是刻意延後的範圍：修正需要替四個 port 實作都加上取消參數並讓 NDJSON 迴圈能中途跳出，牽動的是 port 合約本身，留給下一版。
